...
extension VideoPlayerStatePlaybackControls on VideoPlayerState {
  ...
  void pause() {
    if (_status == PlayerStatus.playing) {
      ...
      // Stop UI update Ticker immediately
      _uiUpdateTicker?.stop();
      
      // Pause playback immediately (don't wait for async operations)
      player.pauseDirectly().catchError((e) {
        debugPrint('[VideoPlayerState] pauseDirectly() 调用失败: $e');
        // Fallback to traditional method
        player.state = PlaybackState.paused;
      });

      // Do other operations asynchronously without blocking
      Future.microtask(() {
        // Jellyfin sync
        if (_currentVideoPath != null &&
            _currentVideoPath!.startsWith('jellyfin://')) {
  ...
  void play() {
    ...
    if (hasVideo &&
      ...
      // Ensure UI update Ticker is running
      if (_uiUpdateTicker == null) {
        _startUiUpdateTimer();
      }
      ...
      // Start playback immediately (don't wait for async operations)
      player.playDirectly().catchError((e) {
        debugPrint('[VideoPlayerState] playDirectly() 调用失败: $e');
        // Fallback to traditional method
        player.state = PlaybackState.playing;
      });
      
      // Do volume check and other operations asynchronously without blocking
      Future.microtask(() {
        _ensurePlayerVolumeMatchesPlatformPolicy();
        
        // Submit watch history in background
        _submitWatchHistoryToDandanplay();
        
        // Screenshot and decoder info updates in background
